...
Sound Property sr_FHUMoanOralMarker  Auto  
...
Event OnUpdate()
	if player.isinfaction(inflaterAnimatingFaction)
		FHUmoanSoundEffect(player as ObjectReference, Utility.randomint(1,2))
	endif
EndEvent
...
Function FHUmoanSoundEffect(ObjectReference aksource, int type)
if sr_MoanSound.getvalue() == 1
	...
	if sr_SexlabMoanSound.getvalue() == 1
		...
		if type == 1
			sr_FHUMoanMildMarker.play(aksource)
		elseif type == 2
			sr_FHUMoanHardMarker.play(aksource)
		elseif type == 3
...
Function InflateTo(Actor akActor, int h, float time, float targetLevel = -1.0, String callback = "")
	if targetLevel <= 0.0
...
Function InflateDeflate(Actor akActor, Bool Inflation, int h, float time, float targetLevel = -1.0, String callback = "")
	if targetLevel <= 0.0
		targetLevel = config.maxInflation
	endIf
	int poolMask = 0
...
Function Absorbto(Actor akActor, int h, float time, float targetLevel = -1.0, String callback = "")
	if targetLevel <= 0.0
		targetLevel = config.maxInflation
	endIf
	int poolMask = 0
...
;	log("Starting animation for " + akActor.GetLeveledActorBase().GetName())
	if Cumtype < 3
		if akactor.getfactionrank(InflateFaction) >= 50
	...
	If Utility.RandomInt(0, 99) < 40 && sr_TongueEffect.getvalue() == 1
		...
		EmotionWhenLeakage(akActor, 1)
	Else
		EmotionWhenLeakage(akActor, 0)
	EndIf
...
Function EmotionWhenLeakage(actor akActor, int Tongue)
...
int Function QueueActor(Actor a, bool inflate, int poolmask, float amount, float time = 6.0, String callback = "", int animate = 0)
...
int Function QueueAbsorbActor(Actor a, bool inflate, int poolmask, float amount, float time = 6.0, String callback = "", int animate = 0)
;	log("Queueing " + a.GetLeveledActorBase().GetName())
	int m = threads.length
	int i = 0
	int res = -1
			...
			While n > 0
				...
				while queued < threads.length && n > 0
					...
					;	log("Deflate actor " + a.GetLeveledActorBase().GetName() + "? Anal: " + deflateAnal +", Vaginal: " + deflateVag)
						...
						int tid = 0
						float defTime = Utility.RandomFloat(4.0, 8.0) * config.animMult
...
int thisSlot = 0x01
	...
	if (Math.LogicalAnd(thisSlot, slotsChecked) != thisSlot)
		wornforms[index] = target.GetWornForm(thisSlot) as Armor
		;if (thisArmor.Haskeyword(ArmorClothing) || thisArmor.Haskeyword(ArmorLight) || thisArmor.Haskeyword(ArmorHeavy)); SLA Keywords?
		if wornforms[index]
			Target.UnequipItem(wornforms[index], false, true)
			index += 1
		EndIf
	endif
