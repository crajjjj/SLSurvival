...
/;
	if !ActorRef
		return -1
	endIf
	; Check actor in current thread ActorAlias because
	; some Mods check for the AnimationEnd event to start 
	; new animation with the player and in that point the thread 
	; have not ActorAlias for the actor but still have the actor
	; in the Positions Array. Also the thread is taking more 
	; time to be clean, probably because the Hooks
	int i
	int alt = -1
	while i < Slots.Length
		if Slots[i].FindSlot(ActorRef) != -1
			If Slots[i].GetState() != "Ending"
				return i
			Else
				alt = i
			EndIf
