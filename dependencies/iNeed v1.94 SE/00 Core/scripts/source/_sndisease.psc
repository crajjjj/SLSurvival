...
State Diseased
	...
	Event OnObjectEquipped(Form akBaseObject, ObjectReference akReference)
		If akBaseObject as Potion
			...
			Int iCureDisease = (akBaseObject as Potion).GetCostliestEffectIndex() as Int
			If (iPotion > -1 || akBaseObject == _SNQuest._SNCureDisease || _SNQuest._SNMagicEffect_CureList.HasForm((akBaseObject as Potion).GetNthEffectMagicEffect(iCureDisease)))
				If _SNQuest._SNDiseaseStage.GetValue() as Int > 0
					If (iPotion == _SNQuest.DiseaseType || akBaseObject == _SNQuest._SNCureDisease || _SNQuest._SNMagicEffect_CureList.HasForm((akBaseObject as Potion).GetNthEffectMagicEffect(iCureDisease)))
						_SNQuest.DiseaseCuring = True
						ResetTime()
						Debug.Notification(_SNQuest.DiseaseCureText)
					EndIf
