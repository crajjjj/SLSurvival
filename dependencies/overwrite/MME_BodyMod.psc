...
Function SetNodeScale(Actor akActor, string nodeName, float value, bool isFemale)
	...
	If Game.GetModbyName("SexLab Inflation Framework.esp") != 255
		...
		If (sKey != "")
			int SLIF_inflate1 = ModEvent.Create("SLIF_inflate")
			If (SLIF_inflate1)
				ModEvent.PushForm(SLIF_inflate1, akActor)
				ModEvent.PushString(SLIF_inflate1, SLIF_modName)
				ModEvent.PushString(SLIF_inflate1, sKey)
				ModEvent.PushFloat(SLIF_inflate1, value)
				ModEvent.PushString(SLIF_inflate1, modName)
				ModEvent.Send(SLIF_inflate1)
			EndIf
		EndIf
