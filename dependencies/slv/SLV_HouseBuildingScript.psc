Scriptname SLV_HouseBuildingScript extends Quest Conditional
...
function RemodelEntryRoom(int partHouseLocation, int partID)
	(HouseQuests[partHouseLocation] as SLV_HouseScript).RemodelEntryRoom(partID)
endFunction
...
;	if activeHouseLocation >=0 && activeHouseLocation <=2
		...
		if newHousePartIndex > -1
			...
			(HouseQuests[partHouseLocation] as SLV_HouseScript).BuildHouseExteriorPart(newHousePartIndex)
		else
...
function BuildHouseInteriorPart(int partHouseLocation, int partID, form part, int roomID)
	...
	FormList foundList
	if roomID ==1
		if currentVariantID == "A" || currentVariantID == "a"
			foundList = RoomMasterLists[0]
		elseif currentVariantID == "B" || currentVariantID == "b"
			foundList = RoomMasterLists[1]
		endif
	else
		int slv_roomID = roomID - 1
		foundList = RoomMasterLists[slv_roomID]
	endif
	...
	if newHousePartIndex > -1
		...
		(HouseQuests[partHouseLocation] as SLV_HouseScript).BuildHouseInteriorPart(roomID, currentVariantID, newHousePartIndex)
		; check if parts of the house have been completed (e.g. for children, spouse, etc.)
		UpdateCompletionStatus( (HouseQuests[partHouseLocation] as SLV_HouseScript), roomID, partID )
	else
...
function UpdateCompletionStatus (SLV_HouseScript houseQuest, int roomID, int partID)
	; check for spouse
...
function BuildHousePart(int partHouseLocation, int newHousePartID, form newHousePart, int finishRoomID, int startRoomID, bool bDisableAdditionLayouts)
	...
	SLV_HouseScript myHouse = (HouseQuests[partHouseLocation] as SLV_HouseScript)
...
;			SLV_BuildingObjectScript listObject = listForm as SLV_BuildingObjectScript
...
;		endif
	...
	if newHousePartIndex > -1
		...
		;if finishRoomID > 0  // we skip this part
		if finishRoomID > 1000
			...
			if myHouse.numRoomsCompleted >= 5
				...
				while iHouseIndex < HouseQuests.Length
					if IsHouseComplete(HouseQuests[iHouseIndex] as SLV_HouseScript)
...
bool function IsHouseComplete(SLV_HouseScript myHouse)
...
function HireSteward(int houseIndex, Actor akNewSteward)
	...
	(HouseQuests[houseIndex] as SLV_HouseScript).bHaveSteward = true
...
function DismissSteward(int houseIndex, Actor akSteward)
	...
	if HouseStewards[houseIndex].GetActorRef() == akSteward
		...
		(HouseQuests[houseIndex] as SLV_HouseScript).bHaveSteward = false
	endif
endFunction
...
function HireCarriageDriver(Actor akSteward)
	...
	if houseIndex > -1
		(HouseQuests[houseIndex] as SLV_HouseScript).HireCarriageDriver()
	endif
endFunction
...
function HireBard(Actor akSteward)
	int houseIndex = GetStewardIndex(akSteward)
	if houseIndex > -1
		(HouseQuests[houseIndex] as SLV_HouseScript).HireBard()
	endif
endFunction
...
function DismissBard(Actor akSteward)
	...
	(HouseQuests[houseIndex] as SLV_HouseScript).bBoughtBard = false
...
function BuyGarden(Actor akSteward)
	int houseIndex = GetStewardIndex(akSteward)
	if houseIndex > -1
		(HouseQuests[houseIndex] as SLV_HouseScript).BuyGarden()
	endif
endFunction
...
function BuyCow(Actor akSteward)
	int houseIndex = GetStewardIndex(akSteward)
	if houseIndex > -1
		(HouseQuests[houseIndex] as SLV_HouseScript).BuyCow()
	endif
endFunction
...
function BuyChicken(Actor akSteward)
	int houseIndex = GetStewardIndex(akSteward)
	if houseIndex > -1
		(HouseQuests[houseIndex] as SLV_HouseScript).BuyChicken()
	endif
endFunction
...
function BuyHorse(Actor akSteward)
	int houseIndex = GetStewardIndex(akSteward)
	if houseIndex > -1
		(HouseQuests[houseIndex] as SLV_HouseScript).BuyHorse()
	endif
endFunction
...
function BuyBuildingMaterial(Actor akSteward, int iMaterialType)
	...
	elseif iMaterialType == 1
		...
		(HouseQuests[houseIndex] as SLV_HouseScript).StewardChest.AddItem(BYOHMaterialStoneBlock, BYOHHPAmountStone.GetValueInt())
	elseif iMaterialType == 2
		...
		(HouseQuests[houseIndex] as SLV_HouseScript).StewardChest.AddItem(BYOHMaterialClay, BYOHHPAmountClay.GetValueInt())
	endif
...
function BuyRoomFurniture(Actor akSteward, int iRoomID)
	...
	SLV_HouseScript myHouseQuest = (HouseQuests[houseIndex] as SLV_HouseScript)
...
function EnableRoomFurniture(int iHouseIndex)
	...
	SLV_HouseScript myHouseQuest = (HouseQuests[iHouseIndex] as SLV_HouseScript)
	; get room globals for this house
	FormList roomFlags = myHouseQuest.RoomDoneFlags
...
;					endif
					...
					SLV_BuildingObjectScript baseObject = roomMasterList.GetAt(iCurrentArrayIndex) as SLV_BuildingObjectScript
...
ObjectReference[] function GetRoomEnableList(SLV_HouseScript houseQuest, int iRoomID)
	if iRoomID == 0
		; room 1A
...
ObjectReference[] function GetRoomDisableList(SLV_HouseScript houseQuest, int iRoomID)
	if iRoomID == 0
		; room 1A
...
int function GetFormListIndex(FormList myList, int myID, form myForm = None)
	...
	if myForm && (myForm as SLV_BuildingObjectScript).ID == myID
		debug.trace("GetFormListIndex: starting FAST SEARCH")
...
; when player enters interior, make sure swap triggers are disabled
	if akNewLoc == BYOHHouse1LocationInterior
		(HouseQuests[0] as SLV_HouseScript).DisableInteriorSwapTriggers()
	elseif akNewLoc == BYOHHouse2LocationInterior
		(HouseQuests[1] as SLV_HouseScript).DisableInteriorSwapTriggers()
	elseif akNewLoc == BYOHHouse3LocationInterior
		(HouseQuests[2] as SLV_HouseScript).DisableInteriorSwapTriggers()
	endif
