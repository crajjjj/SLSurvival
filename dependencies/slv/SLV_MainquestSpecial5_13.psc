;BEGIN FRAGMENT CODE - Do not edit anything between this and the end comment
;NEXT FRAGMENT INDEX 1
Scriptname SLV_MainquestSpecial5_13 Extends TopicInfo Hidden

;BEGIN FRAGMENT Fragment_0
Function Fragment_0(ObjectReference akSpeakerRef)
Actor akSpeaker = akSpeakerRef as Actor
;BEGIN CODE
GetOwningQuest().SetObjectiveCompleted(6000)

if Game.getplayer().isInFaction(SlaverunSlaverFaction)
	getowningquest().setstage(6500)
else
	getowningquest().setstage(6200)
endif

SexSlave.GetActorRef().enable()
SexSlave.GetActorRef().moveto(Game.GetPlayer())
ActorUtil.AddPackageOverride(SexSlave.GetActorRef(), FollowPlayer ,100)
SexSlave.GetActorRef().evaluatePackage()

SexTourist3a.GetActorRef().enable()
SexTourist3b.GetActorRef().enable()
;END CODE
EndFunction
;END FRAGMENT
;END FRAGMENT CODE - Do not edit anything between this and the begin comment
Faction Property SlaverunSlaverFaction Auto
ReferenceAlias Property SexSlave Auto 
ReferenceAlias Property SexTourist3a Auto 
ReferenceAlias Property SexTourist3b Auto 
Package Property FollowPlayer Auto


