...
Function TameCreature(Actor akActor)
	If !akActor.IsInFaction(_SLS_AnimalFriendFaction)
		...
		If AliasSelect != None
			...
			akActor.SetPlayerTeammate(abCanDoFavor = false) ; [Teammate patch] friend follows/fights with the player; abCanDoFavor=false suppresses favor prompts
...
Function DismissFriend(Actor akSpeaker, Bool MoveHome = false)
	...
	If AliasSelect.GetReference() != None
		...
		akSpeaker.SetPlayerTeammate(false) ; [Teammate patch] clear teammate flag when the friend is dismissed
