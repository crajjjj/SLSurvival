...
event OnPageReset(string page)
	...
	ElseIf page == "$SLS_pTrauma"
		...
		If _SLS_IntAudioUtil.GetIsInstalled()
			StorageUtil.SetIntValue(Self, "AudioVoiceVolOID_S", AddSliderOption("AudioUtil voice volume", Util.AudioVoiceVol * 100.0, "{0}%"))
			StorageUtil.SetIntValue(Self, "AudioSfxVolOID_S", AddSliderOption("AudioUtil SFX volume", Util.AudioSfxVol * 100.0, "{0}%"))
		Else
			; Not rendered: park the stored OIDs on -1 so a stale id can't shadow another option
			StorageUtil.SetIntValue(Self, "AudioVoiceVolOID_S", -1)
			StorageUtil.SetIntValue(Self, "AudioSfxVolOID_S", -1)
		EndIf
...
event OnOptionHighlight(int option)
	...
	ElseIf StorageUtil.GetStringValue(Self, "CurrentPage") == "$SLS_pTrauma" ; <----------------->
		...
		ElseIf Option == StorageUtil.GetIntValue(Self, "AudioVoiceVolOID_S")
			SetInfoText("Volume of the voiced lines SLS plays through AudioUtil (moans, pain grunts, swallow reactions). Separate from the voice pack mod's own sliders.")
		ElseIf Option == StorageUtil.GetIntValue(Self, "AudioSfxVolOID_S")
			SetInfoText("Volume of the impacts and gulps SLS plays through AudioUtil. Multiplies with the pain/hit volume sliders above.")
...
Event OnOptionSliderOpen(int option)
	...
	ElseIf StorageUtil.GetStringValue(Self, "CurrentPage") == "$SLS_pTrauma" ; <----------------->
		...
		ElseIf Option == StorageUtil.GetIntValue(Self, "AudioVoiceVolOID_S")
			SetSliderOptions(Value = Util.AudioVoiceVol * 100.0, Default = 100.0, Min = 0.0, Max = 100.0, Interval = 1.0)
		ElseIf Option == StorageUtil.GetIntValue(Self, "AudioSfxVolOID_S")
			SetSliderOptions(Value = Util.AudioSfxVol * 100.0, Default = 100.0, Min = 0.0, Max = 100.0, Interval = 1.0)
		ElseIf Option == StorageUtil.GetIntValue(Self, "TraumaCountMaxPlayerOID_S")
...
Event OnOptionSliderAccept(int option, float value)
	...
	ElseIf StorageUtil.GetStringValue(Self, "CurrentPage") == "$SLS_pTrauma" ; <----------------->
		...
		ElseIf Option == StorageUtil.GetIntValue(Self, "AudioVoiceVolOID_S")
			Util.AudioVoiceVol = value / 100.0
			Util.ApplyAudioUtilVolumes()
			SetSliderOptionValue(StorageUtil.GetIntValue(Self, "AudioVoiceVolOID_S"), Util.AudioVoiceVol * 100.0)
		ElseIf Option == StorageUtil.GetIntValue(Self, "AudioSfxVolOID_S")
			Util.AudioSfxVol = value / 100.0
			Util.ApplyAudioUtilVolumes()
			SetSliderOptionValue(StorageUtil.GetIntValue(Self, "AudioSfxVolOID_S"), Util.AudioSfxVol * 100.0)
		ElseIf Option == StorageUtil.GetIntValue(Self, "TraumaCountMaxPlayerOID_S")
...
Function LoadSettings()
	If ShowMessage("Are you sure you want to overwrite your current settings with the settings save in the json file?")
		...
		Util.AudioVoiceVol = JsonUtil.GetFloatValue("SL Survival/Settings.json", "Util.AudioVoiceVol", missing = 1.0)
		Util.AudioSfxVol = JsonUtil.GetFloatValue("SL Survival/Settings.json", "Util.AudioSfxVol", missing = 1.0)
		Util.ApplyAudioUtilVolumes() ; group volumes are DLL session state - push the imported values now
...
Function SaveSettings()
	If ShowMessage("Are you sure you want to overwrite the settings saved in the json file with your current settings?")
		...
		JsonUtil.SetFloatValue("SL Survival/Settings.json", "Util.AudioVoiceVol", Util.AudioVoiceVol)
		JsonUtil.SetFloatValue("SL Survival/Settings.json", "Util.AudioSfxVol", Util.AudioSfxVol)
